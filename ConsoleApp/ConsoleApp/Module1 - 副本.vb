...
Module Module1
  ...
  Sub createDocumnet()
    ...
    str = Left(str, Len(str) - 1)
      str = "[" + str + "]"
    Dim fs As FileStream = File.Create(doc.FilePath + doc.FileName + ".font.json")
    Dim info As Byte() = New UTF8Encoding(True).GetBytes(str)
      fs.Write(info, 0, info.Length)
      fs.Close()
    '文件写入完成
    log("finish", "0001")
    MsgBox("文件保存到：" + doc.FilePath + doc.FileName + ".font.json")

    ' Dim cmd As String
    '*打开资源管理器 焦点 在【MyDotNetFiles】文件夹
    ' cmd = "cmd /k explorer /select," + doc.FilePath
    'Call Shell(cmd)

    '输出参数
    ' Console.Write(doc.FilePath)
